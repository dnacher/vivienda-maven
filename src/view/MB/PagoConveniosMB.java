...
public class PagoConveniosMB implements Initializable {
	...
	public void initialize(URL url, ResourceBundle rb) {
		task();
		try {
//			btnAgregar.setDisable(UtilsGenerals.traePermisos(MenuAdministracion.PagoConvenios.getPagina(), Constantes.PERMISO_OPERADOR));
		...
		chkBonificacion.selectedProperty().addListener(new ChangeListener() {
			...
			public void changed(ObservableValue observable, Object oldValue, Object newValue) {
				...
				if (chkBonificacion.isSelected()) {
					descuento = UtilsGenerals.calculaBonificacion(convenio.getReglabonificacion(), sugerido);
				} else {
					descuento = 0;
				}
...
//					gcb.guardar(gc);
//					viviendas.vivienda.Viviendas.confirmacion = false;
//					cv.creaVentanaNotificacionCorrecto();
//				} else {
//					lblInfo.setText(ConstantesErrores.DEBE_SELECCIONAR_UNIDAD);
//				}
