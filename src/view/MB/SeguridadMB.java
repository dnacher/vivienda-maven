...
import persistence.enums.MenuAdministracion;
import persistence.enums.MenuConfiguracion;
import persistence.enums.MenuMantenimiento;
import persistence.enums.MenuPrincipal;
...
public class SeguridadMB implements Initializable {
    ...
    PermisoUsuario administracion;
    PermisoUsuario inicio;
    PermisoUsuario mantenimiento;
    PermisoUsuario configuracion;
    PermisoUsuario reportes;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        try {
//            tableData.setDisable(UtilsGenerals.traePermisos(MenuConfiguracion.Seguridad.getPagina(), Constantes.PERMISO_ADMIN));
            lblnoVisibles(false);
            task();
            atras(null);
...
//                }
    ...
    private void cargaComboPagina() {
        ...
        for (PermisoUsuario pu : Viviendas.listaPermisos) {
            if (pu.getPermiso() == 7) {
    ...
    public void nuevoGrupo() {
        if (tableData.getSelectionModel().getSelectedItem() != null) {
            ...
            administracion = new PermisoUsuario();
            administracion.setPagina(MenuPrincipal.Administracion.getPagina());
            administracion.setTipoUsuario(tu);
            administracion.setPermiso(7);
